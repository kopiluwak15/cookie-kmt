...
export type ConceptSurveyPayload = {
  symptoms: string[]
...
type ConceptDraft = ConceptSurveyPayload & { step: number }

function loadDraft(storageKey: string): ConceptDraft | null {
...
interface ConceptSurveyFormProps {
  /** 送信ハンドラ。payload を受け取り完了したら resolve */
  onSubmit: (payload: ConceptSurveyPayload) => Promise<void>
  /** ドラフト保存用キー（重複させない） */
  storageKey?: string
}

export function ConceptSurveyForm({
  onSubmit,
  storageKey = 'liff_concept_draft_v1',
}: ConceptSurveyFormProps) {
